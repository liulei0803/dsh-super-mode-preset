...
| Persona Depth | Basic | **Principal Engineer** | ⭐⭐⭐ |
...
4. Select "超级模式" (Super Mode) from the preset selector when creating a new session

### 🎯 Use Cases

- 🔥 Complex architecture design and refactoring
- 🔥 Large-scale changes across multiple files
- 🔥 Deep reasoning for difficult bugs
- 🔥 Full-stack feature development (frontend + backend + infrastructure)
...
**超级模式** 是 DeepSeek Harness (DSH) 的终极增强 Agent 预设，将你的编码 Agent 转变为首席工程师级别。具备深度推理能力、优化的上下文保持策略和强化的多代理协作。

### ✨ 核心特性

- **首席工程师人设**：第一性原理思维、系统级感知、主动分析能力
- **全栈精通**：深度掌握 TypeScript/Python/Rust/Go、React/Vue/Next.js、Docker/K8s、数据库和 AI/ML
- **增强上下文保持**：2 倍压缩阈值（16KB vs 8KB），保留更多对话历史
- **强化协作能力**：128 轮 Ralph 迭代（vs 64），更好的多代理协调
- **优化工具配置**：更大的指令上下文（128KB vs 64KB），支持并行任务

### 📊 性能对比

| 特性 | 标准模式 | 超级模式 | 提升 |
|------|---------|---------|------|
| 指令上下文 | 64KB | **128KB** | +100% |
| 压缩阈值 | 8KB | **16KB** | +100% |
| 头部保留 | 4KB | **8KB** | +100% |
| 尾部保留 | 1KB | **4KB** | +300% |
| Ralph 迭代 | 64 轮 | **128 轮** | +100% |
| 人设深度 | 基础 | **首席工程师** | ⭐⭐⭐ |

### 💰 Token 消耗

- **每轮请求额外开销**：~2,000-5,000 tokens（比标准模式多 ~20-30%）
- **权衡**：更高的 token 成本，换来更强的推理和上下文记忆
- **适用场景**：复杂架构设计、大规模重构、深度调试、全栈开发
...
2. 将 `super-mode` 目录复制到 DSH 预设文件夹：
   - **Windows**: `%USERPROFILE%\.dsh\.agent-presets\super-mode\`
   - **macOS/Linux**: `~/.dsh/.agent-presets/super-mode/`

3. 重启 DSH Desktop

4. 创建新会话时，在预设选择器中选择「超级模式」
...
Created for the DeepSeek Harness community. Built with ❤️ for developers who demand excellence.

为 DeepSeek Harness 社区创建。为追求卓越的开发者用心打造。
